...
| A1 DLC | sideSlipMax [°] | 3.02 | **2.98** | -1.2% | 6/6 |
| A1 | LTR_max | 0.864 | **0.785** | -9.1% | 3.46/5 |
| A1 | lateralDevMax [m] | 1.827 | 1.986 | +8.7% | 0/4 |
...
| D1 DLC+Brake | sideSlipMax [°] | 4.91 | **4.29** | -12.6% | 3.71/4 |
| D1 | LTR_max | 0.864 | **0.785** | -9.1% | 1.38/2 |
| D1 | lateralDevMax [m] | 1.827 | 1.986 | +8.7% | 0/2 |
| | | | | **합계** | **61.12/70** |
...
**A1 ISO 3888-1 DLC (9.46/15)**: sideSlip은 2.98°로 만점(6/6)이다. DLC 모드에서는 AFS 크기를 3°로 제한한다. LTR은 0.785로 목표 0.6 미달 (3.46/5), lateralDev는 ON(1.986 m)이 OFF(1.827 m)보다 크다 (0/4). §5에서 잔여 감점 항목을 정리한다.
...
만점을 달성하지 못한 KPI는 A1/D1 lateralDev, A1/D1 LTR, D1 sideSlip, A4 understeerGradient이다. 각 항목을 현재 제어기 입력과 출력 범위 기준으로 정리한다.
...
결과에서도 OFF(1.827 m) → ON(1.986 m)으로 lateralDev가 증가하였다. 본 설계는 yaw rate와 slip angle을 제어 대상으로 두었고, 경로 오차는 직접 피드백하지 않았다.

#### 5.2.2 LTR_max (A1: 3.46/5, D1: 1.38/2)
...
여기서 $h_{cog}$는 무게중심 높이, $a_y$는 횡가속도, $g$는 중력가속도, $t_{track}$은 track width이다. $h_{cog}$, $t_{track}$은 차량 기하 파라미터이고, 제어기 출력에는 roll stiffness를 직접 바꾸는 액추에이터가 없다. 현재 수직 제어기는 CDC 감쇠 계수만 출력한다. 따라서 본 설계에서는 LTR을 직접 제어하지 않고, CDC는 감쇠 계수 범위 안에서만 동작한다.
...
| A1/D1 LTR | 2.16점 | 출력 액추에이터가 CDC 감쇠 계수로 제한 |
| D1 sideSlip | 0.29점 | 타이어 비선형 영역 |
| A4 understeer | 0.44점 | AFS 개입과 정상선회 KPI 간 상충 |

합계 미달은 8.89점이다. 현재 제출 설계는 A3, A7, B1을 만점으로 유지하고, A1/D1의 slip 및 LTR 부분 점수를 확보하는 방향으로 정리하였다.
...
**Claude Code** (Anthropic, Claude Opus 4) 및 **Codex**를 다음 범위에서 활용하였다:
